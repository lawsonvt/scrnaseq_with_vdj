...
out_dir <- "results/microglia_subset.diff_exp_analysis.pseudobulk/"
dir.create(out_dir, showWarnings = F)


# load in named seurat object
seu_obj <- readRDS("results/seurat_cluster_naming.microglia_subset/cell_named.microglia_subset.seurat.RDS")
...
cells <- unique(metadata$microglia_cell_name)

table(metadata$orig.ident,
      metadata$microglia_cell_name)
...
seu_pseudo <- AggregateExpression(
  ...
  assays = "RNA",
  slot = "counts",
  group.by = c("microglia_cell_name","orig.ident")
)
...
ps19ko_minus_ps19wt.de_results <- lapply(cells, function(cell) {
  ...
  # remove any NAs due to independent filtering
