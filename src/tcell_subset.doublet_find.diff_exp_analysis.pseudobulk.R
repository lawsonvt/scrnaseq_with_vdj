...
out_dir <- "results/tcell_subset.doublet_find.diff_exp_analysis.pseudobulk/"
dir.create(out_dir, showWarnings = F)


# load in named seurat object
seu_obj <- readRDS("results/tcell_split.doublet_find/tcell_subset.named.seurat.RDS")
...
# rename the cell types for the analysis
metadata$tcell_type <- gsub("\\+", "p", metadata$tcell_type)

seu_obj@meta.data$tcell_type <- gsub("\\+", "p", seu_obj@meta.data$tcell_type)

# get the cell types

cells <- unique(metadata$tcell_type)

# just get the CD t cells
cells <- cells[grepl("(CD)|(NK)", cells)]
...
seu_pseudo <- AggregateExpression(
  ...
  assays = "RNA",
  slot = "counts",
  group.by = c("tcell_type","orig.ident")
)
...
# fix cell names
ps19ko_minus_ps19wt.de_results <- lapply(ps19ko_minus_ps19wt.de_results, function(data) {
  
  data$cell_type <- gsub("p$", "+", data$cell_type)
...
ggsave(paste0(out_dir, "ps19ko_minus_ps19wt.deg_counts.png"), width=4, height=5)
...
ggsave(paste0(out_dir, "ps19ko_minus_ps19wt.volcano_plots.png"), width=8, height=4)
